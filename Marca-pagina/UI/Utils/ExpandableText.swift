...
struct ExpandableText: View {
    ...
    private var moreLessText: String {
        if !truncated {
            ...
            return self.expanded ? " Menos" : " ... Mais"
        }
    }

    init(_ text: String,
         lineLimit: Int,
         font: UIFont = UIFont(name: "Atkinson Hyperlegible Regular", size: 17) ?? UIFont.systemFont(ofSize: 17)) {
        self.text = text
        ...
        ZStack(alignment: .bottomLeading) {
            ...
            if truncated {
                Button(action: {
                    ...
                    HStack {
                        // taking tap on only last line, As it is not possible to get 'see more' location
                        Spacer()
                        Text("")
                    }.opacity(0)
                })
